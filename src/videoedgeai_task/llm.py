...
class MockLLMProvider:
    ...
    def _infer_audience(self, text: str) -> str:
        ...
        if any(w in lowered for w in ("doctor", "patient", "nurse", "hospital", "clinical",
            ...
            return (
                "Healthcare teams who need to hand off critical patient context "
        ...
        if any(w in lowered for w in ("carbon", "climate", "sustainability", "emission",
            ...
            return (
                "Small business owners and sustainability leads working toward "
                "measurable emissions goals."
            )
    ...
    def _infer_value(self, text: str, audience: str) -> str:
        ...
        if any(w in lowered for w in ("doctor", "patient", "shift", "handoff", "clinical")):
            return (
                "The workflow standardises patient handoffs so critical context "
                "is never lost between shifts."
            )
        if any(w in lowered for w in ("sales", "prospect", "crm", "pipeline", "close")):
            return (
                "The tool surfaces the highest-likelihood accounts so the team "
                "closes more in less time."
            )
        if any(w in lowered for w in ("hire", "onboard", "recruit", "talent")):
            return (
                "The system connects new hires to the right internal contacts "
                "faster, reducing ramp-up time."
            )
        if any(w in lowered for w in ("carbon", "climate", "emission", "footprint", "green")):
            return (
                "The service gives small businesses a clear, actionable view of "
                "their carbon footprint without specialist overhead."
            )
        if any(w in lowered for w in ("consumer", "personal", "daily", "habit", "fitness")):
            return (
                "The tool helps users build a consistent routine without the "
                "noise of a full productivity system."
            )
        return (
